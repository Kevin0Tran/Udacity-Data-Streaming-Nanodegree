...
spark = SparkSession.builder.appName("spark-redis-kafka").getOrCreate()
...
kafka_redis_server_stream_df.withColumn("value", from_json("value", redis_server_schema)) \
.select(col("value.*")) \
...
zsetEntriesEncoded = spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zsetEntriesDecoded = zsetEntriesEncoded.withColumn("encodedCustomer", unbase64(zsetEntriesEncoded.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
zsetEntriesDecoded.withColumn("customer", from_json("encodedCustomer",customer_schema)).select(col("customer.*")).createOrReplaceTempView("CustomerRecords")
